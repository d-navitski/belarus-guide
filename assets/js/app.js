...
APP = {
    init: function() {
        if (this.features.hasTouch) {
            $(this.elements.$html).addClass('touch');
        } else {
            $(this.elements.$html).addClass('no-touch');
        }
    },
    features: {
        hasTouch: (('ontouchstart' in window) || (navigator.msMaxTouchPoints > 1))
    },
    selectors: {
        elHtml: 'html',
        elBody: 'body'
...
var header = (function() {
    ...
    APP.header = {
        init: function() {
            window.addEventListener('scroll', this.handlers.parallax);

            this.handlers.addShowProfileListener();
        ...
        handlers: {
            parallax: function() {
                ...
                var headerContHeight = APP.header.elements.$headerContainer
                    .outerHeight();
                var TIME = 0.5,
                    DISTANCE = 20;

                var tween = function tween() {
                    TweenLite.to(APP.header.elements.$headerContainer,
                        TIME, {
                            y: -fromTop / DISTANCE,
                            overwrite: true,
                            force3D: true
                ...
                if (fromTop <= headerContHeight && winWidth > 690) {
                    setTimeout(tween(), 10);
                }
            },
            addShowProfileListener: function() {
                var showProfileTrigger = document.querySelectorAll(APP.header.selectors.jsTrgShowPrfl);

                for (var i = 0, l = APP.header.elements.$jsTrgShowPrfl
                        .length; i < l; i++) {
                    showProfileTrigger[i].addEventListener(
                        'click', APP.header.handlers.showProfile
                    );
                }
            },
            showProfile: function() {
                ...
                var $profile = $(this).parents('.avatar-wrapper');
        ...
        selectors: {
            ...
            // javascript triggers on navigation
            jsTrgShowPrfl: '.js-show-profile'
        },
        elements: {
            $headerContainer: $('.header-container'),
            $weatherBlock: $('.weather-block'),
...
APP.nav = {
	selectors: {
		...
		// javascript triggers in navigation
		jsTrgOpnNav: '.js-open-menu',
		jsTrgOpnSubNav: '.js-open-subnav'
	},

	openSubNav: function () {
		var self = this;

		$(APP.nav.selectors.jsTrgOpnSubNav).click(function () {
			var $subnav = $(this).find(APP.nav.selectors.elSubNav);
			$subnav.slideToggle();
		});

		$(APP.nav.selectors.jsTrgOpnNav).click(function () {
			var $nav = $(this).siblings(APP.nav.selectors.elNav);
			$nav.slideToggle();
		});
	}
};
...
$(function() {
    ...
    var headerContHeight = $headerCont.outerHeight();
    var $weatherBlock = $('.weather-block');

    // Javascript trigger vars
    var $jsOpenSubnav = $('.js-open-subnav');
    var $jsShowProfile = $('.js-show-profile');
    var $jsOpenMenu = $('.js-open-menu');
    var $responsiveTable = $('.js-responsive');
    ...
    if (lang === 'ru') {
        $.ajax({
                url: 'http://api.openweathermap.org/data/2.5/weather?q=Minsk&units=metric&lang=ru',
                dataType: 'json',
            })
            .done(function(data) {
                var icon;
                var description = '<span class="weather-description">' + data.weather[0].description +
                    '</span>';
                var temp = '<span class="weather-temp">' + Math.round(data.main.temp) +
                    '&nbsp;&deg;C' + '</span>';
                switch (data.weather[0].icon) {
                    case '01d':
                        icon = '<span class="weather-icon icon-sun"></span>';
                        break;
                    case '01n':
                        icon = '<span class="weather-icon icon-moon"></span>';
                        break;
                    case '02d':
                        icon =
                            '<span class="weather-icon icon-cloud"></span><span class="weather-icon icon-sunny"></span>';
                        break;
                    case '02n':
                        icon =
                            '<span class="weather-icon icon-cloud"></span><span class="weather-icon icon-night"></span>';
                        break;
                    case '03d':
                        icon = '<span class="weather-icon icon-cloud"></span>';
                        break;
                    case '03n':
                        icon = '<span class="weather-icon icon-cloud"></span>';
                        break;
                    ...
                    case '09d':
                        icon =
                            '<span class="weather-icon icon-basecloud"></span><span class="weather-icon icon-rainy"></span>';
                        break;
                    case '09n':
                        icon =
                            '<span class="weather-icon icon-basecloud"></span><span class="weather-icon icon-rainy"></span>';
                    ...
                    case '10d':
                        icon =
                            '<span class="weather-icon icon-basecloud"></span><span class="weather-icon icon-rainy"></span><span class="weather-icon icon-sunny"></span>';
                        break;
                    case '10n':
                        icon =
                            '<span class="weather-icon icon-basecloud"></span><span class="weather-icon icon-rainy"></span><span class="weather-icon icon-night"></span>';
                        break;
                    case '11d':
                        icon =
                            '<span class="weather-icon icon-basecloud"></span><span class="weather-icon icon-thunder"></span>';
                        break;
                    case '11n':
                        icon =
                            '<span class="weather-icon icon-basecloud"></span><span class="weather-icon icon-thunder"></span>';
                    ...
                    case '13d':
                        icon =
                            '<span class="weather-icon icon-basecloud"></span><span class="weather-icon icon-snowy"></span>';
                        break;
                    case '13n':
                        icon =
                            '<span class="weather-icon icon-basecloud"></span><span class="weather-icon icon-snowy"></span>';
                        break;
                    case '50d':
                        icon = '<span class="weather-icon icon-mist"></span>';
                        break;
                    case '50n':
                        icon = '<span class="weather-icon icon-mist"></span>';
                        break;
                }
                $weatherBlock.html(icon + description + temp);
            })
            .fail(function() {
                console.log("error");
            });
        ...
        $.ajax({
                url: 'http://api.openweathermap.org/data/2.5/weather?q=Minsk&units=metric&lang=en',
                dataType: 'json',
            })
            .done(function(data) {
                var icon;
    ...
    $('.js-ec-toggle').on('click', function(event) {
        event.preventDefault();
